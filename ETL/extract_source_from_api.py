...
def handle_drivers(api_endpoint) -> pd.DataFrame:
...
def handle_standingList(api_endpoint) -> pd.DataFrame:
    url = f'https://ergast.com/api/f1/{api_endpoint}.json'
    response = requests.api.get(url)
...
def handle_races(api_endpoint) -> pd.DataFrame:
    url = f'https://ergast.com/api/f1/{api_endpoint}.json'
    response = requests.api.get(url)
...
def handle_circuits(api_endpoint) -> pd.DataFrame:
    url = f'https://ergast.com/api/f1/{api_endpoint}.json'
    response = requests.api.get(url)
